...
server <- function(input, output, session) {
  ...
  observeEvent(input$dashboard, {
    ...
    # Correct file path for rendered HTML
    output_path <- file.path("/srv/shiny-server/init-scripts", paste0(isolate(input$db_name_input), ".html"))
    ...
    tryCatch({
      ...
      # Dynamically create the render button after the connection is stablished
      output$dashboard_link <- renderUI({
        if (connection_opened()){
          req(input$dashboard)  # Ensure input exists
          
          tags$a(
            href = paste0("/init-scripts/", isolate(input$db_name_input), ".html"),
            target = "_blank",
      ...
      removeModal()
      shiny::showNotification("Failed to render dashboard: ", e$message, type = "error")
    })
  })
